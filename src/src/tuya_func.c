...
void on_messages(tuya_mqtt_context_t *context, void *user_data,
  ...
  case THING_TYPE_ACTION_EXECUTE:
    ...
    if(strcmp(action_code->valuestring, "on")){
      ...
      status = set_device_pin_state(cJSON_GetObjectItem(action_params, "port")->valuestring, ON, cJSON_GetObjectItem(action_params, "pin")->valueint, &out);
    } 
    else if(strcmp(action_code->valuestring, "off")){
      struct data_output out;
      status = set_device_pin_state(cJSON_GetObjectItem(action_params, "port")->valuestring, OFF, cJSON_GetObjectItem(action_params, "pin")->valueint, &out);
    }
    if (status != 0) {
