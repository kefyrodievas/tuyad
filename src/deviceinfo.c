...
struct meminfo get_mem_info() {
  struct sysinfo info;
  sysinfo(&info);
  struct meminfo res = {info.totalram * info.mem_unit,
...
int get_cpu_info(union cpuinfo *info) { // to be fixed, percentages are weird
  ...
  // info->usage = (cpu_work / info->cpustat.idle) * 100;
  return 0;
}
...
char *device_data_to_json(struct meminfo mem_info, union cpuinfo cpu_info,
  ...
  if (memory == NULL)
    goto end;
  cJSON_AddNumberToObject(memory, "total", mem_info.total_mem);
  cJSON_AddNumberToObject(memory, "free", mem_info.free_mem);
